...
def pack_n_press(A, b, H, L, wr, flag = 1, debug = True):
  ...
  #pmodel.addConstrs(Lp[k,r] <= wr[k][r] for k in K for r in R if wr[k][r] > 0.1)
  ...
  if pmodel.status == GRB.INFEASIBLE:
    ...
    return None
  Lp = np.zeros((len(K),len(R)))
  if debug == True:
    for k in K:
      for r in R:
        for j in J:
          if x[j,k,r].X > 0.1:
            Lp[k,r] = max(Lp[k,r],L[j])
        print("press",k,"region",r,"length",int(Lp[k,r]),"height",int(h[k,r].X/45.0),"waste",omega[k,r].X/1000/1000,'m^2')
        for j in J:
          if x[j,k,r].X > 0.1:
            for i in I:
              if A[i,j] > 0.1:
                print("item",i,"waste=", int(H[j]*(Lp[k,r]-L[j])*x[j,k,r].X)/1000/1000, "in pattern", j, "of width", L[j], "used", A[i,j], "times and order is: ",b[i], "/", b[i], "delta =",int(delta[i].X))
