...
# Include here all the core functions like Mutate, Selection, Objective, Search, etc.
...
def Objective(merged):
    area_needed = merged.data.area
    area_press = GlulamConfig.MAX_ROLL_WIDTH * GlulamConfig.MAX_HEIGHT_LAYERS * GlulamConfig.LAYER_HEIGHT / 1e6
    min_presses = np.ceil(area_needed / area_press).astype(int) - 1

    press = GlulamPackagingProcessor(merged, min_presses)
    objective = (None, None)
...
def Search(data, x=None, max_generations=100, alpha=0.1, sigma0=5, lamba=10,
           n_max=GlulamConfig.MAX_ROLL_WIDTH - GlulamConfig.ROLL_WIDTH_TOLERANCE, dn=GlulamConfig.ROLL_WIDTH_TOLERANCE,
           nmin=0):
